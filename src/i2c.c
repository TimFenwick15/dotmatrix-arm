...
void I2C_bTask(void)
{
    ...
    uint16_t I2C_ADDRESS = 0xD0;
    uint8_t buf[8];
    HAL_StatusTypeDef ret;
    if (doOnce)
    {
        buf[0] = decToBcd(0); // sec
        buf[1] = decToBcd(50); // min
        buf[2] = decToBcd(11); // hour
        buf[3] = decToBcd(6); // day of week
        buf[4] = decToBcd(6); // day of month
        buf[5] = decToBcd(11); // month
        buf[6] = decToBcd(21); // year
        ret = HAL_I2C_Mem_Write(&I2cHandle, I2C_ADDRESS, 0, 1, buf, 7, HAL_MAX_DELAY);
        doOnce = false;
    }
    else
    {
    ...
    if (ret == HAL_OK)
    {
        ...
        ret = HAL_I2C_Mem_Read(&I2cHandle, I2C_ADDRESS, 0, 1, buf, 8, HAL_MAX_DELAY);
        if (ret == HAL_OK)
        {
            seconds = bcdToDec(buf[0]);
            minutes = bcdToDec(buf[1]);
            hour = bcdToDec(buf[2]);
            dayofweek = bcdToDec(buf[3]);
            dayofmonth = bcdToDec(buf[4]);
            month = bcdToDec(buf[5]);
            year = bcdToDec(buf[6]);
        }
    }
